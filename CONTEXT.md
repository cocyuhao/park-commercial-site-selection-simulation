...
- 当前正在做：已按普通用户路径检查项目总览、节点清单、空间地图、资料导入、资料闭合中心、分析报告和专家 AI 工作台。
- 上次停在：主导航均可打开，但发现地图搜索异步完成会把用户从其他页面拉回地图；URL hash 改成 `#ai` 时页面仍停在地图；空 AI 会话仍可点“生成报告”；节点草案被历史测试数据污染。
- 关键决定：地图搜索完成后只刷新地图数据，不再强制切回地图页；新增 hashchange 路由；空会话禁用生成报告；节点草案按来源、名称、位置和业态去重。
- 当前验证：`node --check` 通过；`py -m py_compile` 通过；项目总门禁 `checks=723 failures=0`；Selenium/Chrome 截图保存到 `90_p6_expert_dashboard/qa/user_flow_check_ai_20260603.png`。
- 当前边界：运行态缓存仍有历史测试上传和人工测试节点，页面已从 108 个节点降到 24 个；若要彻底清理测试缓存，需用户明确同意。

# 2026-06-03 GitHub 同步、LFS 补全与门禁修复
- 当前正在做：已确认本地 `main` 与 `origin/main` 完全一致，且通过 `git lfs pull origin main` 补全两个 DWG 大文件，不再只是 Git LFS 指针。
- 上次停在：普通 `git fetch origin main --prune` 后 `HEAD...origin/main` 为 `0 0`；随后发现 DWG 文件只有 133/134 字节，实际是 LFS 指针。
- 关键决定：项目总门禁的 GitHub 检查改为检查当前仓库远端、`HEAD` 与 `origin/main` 同步状态、LFS 跟踪文件；不再依赖本机 `gh` 或匿名逐项扫描外部 fork，避免无关外部限流阻断本项目门禁。
- 当前验证：`node --check` 通过；`py -m py_compile` 通过；项目总门禁 `checks=723 failures=0`；PDF 表格验证 `PASS=4 FAIL=0`；DeepSeek API 验证 `PASS=4 FAIL=0`。
- 当前边界：两个 DWG 已恢复为真实 LFS 大文件且 header 为 `AC1018`，但仍只是待转换源文件；没有可信转换产物前，不得输出坐标、面积、图层、动线或最终选址结论。
