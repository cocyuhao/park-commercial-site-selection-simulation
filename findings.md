...
## 2026-06-03 GitHub 同步事实

- 本地已从 `origin/main` 快进同步到 `4e5cb7b Improve AI workbench sessions and report flow`。
- 同步前本地位于 `74b6aeb`，工作区干净；本轮没有创建分支，也没有执行历史重置。
- 当前机器 `py` 实际指向 Python 3.13，不是旧文档中的 Python 3.12。
- 使用官方 PyPI 源补齐 `requirements.txt` 后，DeepSeek API 验证和 PDF 表格验证均通过。
...
## 2026-06-03 高德 JS 地图改造发现

- 高德 JS API 2.0 地图可以在当前页面真实渲染，Selenium 检查到地图 canvas、AutoNavi 版权、项目标记、候选节点、POI 和结果列表。
- 原地图实现保留了静态图层和自定义拖拽缩放，容易造成“看起来在缩放，实际是假缩放”的误解；本轮已把空间地图主体验切到高德 JS 地图。
- 高德 Web Service 查询和周边 POI 刷新如果链路过长，会让前端误以为搜索失败；本轮把周边 POI 控制为一页，并给前端加超时保护。
- PowerShell 直接发送中文 JSON 仍会造成乱码风险；本轮 Selenium 中文输入改用 Unicode 字符构造，避免把测试脚本问题误判为产品问题。
- 外部项目搜索后可以显示候选节点位置草案，但评分仍不能套用奥森上下文；这类节点只表示待复核位置关系。
- 项目总门禁当前受本机缺少 `gh` 命令阻断，地图专项验证不受影响。
# 2026-06-03 地图/资料/节点/总览联动发现

- 地图竞态的根因是搜索 loading、图层 loading、错误和当前 tab 混在一起；拆成独立状态后，搜索中切换“全部 / 商业 POI / 候选节点”不会清空结果。
- 资料采用后强制打开右侧抽屉会遮挡后续地图和节点操作；改为保持用户当前抽屉状态。
- 节点表单会被 60 秒自动刷新清空；编辑节点时必须暂停自动刷新，保存后再恢复。
- 高德 JS API 在当前本地 Chrome 中返回平台授权不匹配，说明现有 Key 不是适配本地域名的 JS API Key；代码和报告仍不得写完整 Key。
- Selenium 对长页面动态 DOM 很敏感，节点列表刷新后必须重新取元素，不能复用旧引用。

# 2026-06-03 GitHub 同步、LFS 与门禁发现

- 普通 Git 同步完成不等于 LFS 大文件已落地；本轮两个 DWG 起初只有 133/134 字节，内容是 Git LFS 指针，导致 DWG header 门禁读成 `versio`。
- 执行 `git lfs pull origin main` 后，两个 DWG 恢复为 19044528 字节和 153603928 字节，文件头均为 `AC1018`。
- 项目总门禁不应依赖本机必须安装 `gh`，也不应每次匿名逐项请求外部 fork；这会被缺命令或 GitHub 限流阻断，与当前项目同步状态无关。
- 当前更稳的 GitHub 门禁是：确认 `origin` 指向本项目、`HEAD...origin/main` 为 `0 0`、两个 DWG 在 `git lfs ls-files` 中且实际文件头有效。
- `.env` 中项目已支持 `OPENAI_API_KEY` -> `DEEPSEEK_API_KEY`、`AMAP_API_KEY` -> `AMAP_WEB_SERVICE_KEY` 的别名映射，门禁检查也应承认这些别名。
- DeepSeek 首次验证出现 SSL EOF，复跑后 4 项全部通过，属于外部网络瞬时问题，不需要改业务代码。

# 2026-06-03 用户全流程交互发现

- 地图搜索成功后不应强制切回地图页；用户可能已经切到资料或 AI 页面，异步结果只应更新地图数据和状态。
- 应用必须监听 `hashchange`；否则用户直接输入 `#ai` 或使用浏览器前进/后退时，URL 和真实页面会不一致。
- 空 AI 会话不能生成报告；按钮状态必须由当前会话消息数决定，不能用旧的本地 `chatHistory` 兜底。
- 项目计划生成节点必须幂等；同一项目计划、同一位置、同一业态不能反复生成重复草案。
- 历史自动化测试会污染运行态节点和上传资料；正式演示前应清理缓存，但涉及数据删除，需要用户确认。
