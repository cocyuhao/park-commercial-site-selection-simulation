...
### 已完成
- 空间地图主路径已改为真实高德 JS API 2.0 地图，页面可见高德 canvas 和 AutoNavi 版权信息。
- 地图不再用静态图作为展示兜底；静态图层不作为空间地图主体验。
- 地图支持高德原生拖拽、滚轮缩放、按钮缩放。
- 地图容器变化后会触发 `resize` 和 `setFitView`，避免尺寸变化后错位。
- 当前项目位置、候选节点、商业 POI、搜索结果已做成不同视觉标记。
- 点击 POI 可显示信息卡，包含名称、类型、距离、来源和待复核状态。
- 右侧新增简洁结果列表，可点击 POI，同步地图信息卡。
- 搜索失败时统一显示“地图检索暂时失败，可以稍后重试，或先手动输入项目位置。”，不显示后端异常原文。
- 候选节点可随当前搜索项目生成待复核位置草案，但不套用奥森评分。
- 已新增验证报告：`40_quality_evidence/地图_资料_节点_验证报告_20260603.md`。
...
- 失败搜索场景验证通过：输入保留、地图不崩、机器错误隐藏、产品提示显示。
- 项目总门禁未完成：本机缺少 `gh` 命令，脚本停在 GitHub 远端检查处；不是地图代码失败。

### 当前边界
- 地图点位、POI、候选节点仍是待复核草案，不是最终坐标或 DWG 几何。
- 没有把高德 Key 写入代码或报告。

# 2026-06-03 GitHub 最新更新导入

### 已完成
- 已从 GitHub `origin/main` 快进同步到 `4e5cb7b Improve AI workbench sessions and report flow`。
- 本地原提交为 `74b6aeb`，同步前工作区干净；本轮未创建分支、未重置历史。
- 当前机器 `py` 指向 Python 3.13，已按 `requirements.txt` 补齐缺失依赖。

### 验证
- DeepSeek API 验证通过：`PASS=4 FAIL=0`。
- PDF 表格验证通过：`PASS=4 FAIL=0`。

### 当前边界
- 验证脚本更新了验证报告文件，属于本轮运行产物。
- P3 真实输入未闭合前，仍不得输出最终排序、收益预测、ROI 或最终推荐。
...
# 2026-06-03 地图竞态、资料链、节点与总览

- 已把地图搜索 loading 和图层切换 loading 拆开，新增 `activeLayer`、`pendingSearchKeyword` 和 `lastSuccessfulMapContext`，解决搜索“青年湖公园”过程中切换 POI tab 结果区空白的问题。
- 已把资料上传改成项目资料链，支持项目计划一键入口、分项用途、采用/放弃/删除和项目影响说明。
- 已新增节点草案缓存与接口，支持新增、编辑、删除、启停，以及从项目计划生成待复核节点草案。
- 已把项目总览改为动态状态，展示项目计划、地图定位、节点草案、POI、客流/TGI、经营数据和当前阻塞项。
- 验证：Python/Node 语法检查通过；Selenium 竞态 3/3 通过；完整 10 轮已执行但仍受高德 JS Key 授权和长页面动态 DOM 影响。
