...
def report_markdown(basis: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## 1. 结论先行",
        "",
        "这版报告不再把重点放在“还缺什么资料”，而是使用本文件夹内已给数据和老板方法，做出当前可执行的预测与调整方案。需要复核的内容放到后文风险和试运营指标中，不作为主文结论。",
        "",
    ...
    for item in feature["examples"]:
        ...
        lines.append(f"| {item['persona_name']} | {item['state']} | {item['income_price_band']} | {item['action']} | {implication} |")
...
<head>
  ...
  <style>
    ...
    @media (max-width: 760px) {{ main {{ padding: 22px 14px 56px; }} table {{ display: block; overflow-x: auto; }} h1 {{ font-size: 27px; }} }}
  </style>
</head>
