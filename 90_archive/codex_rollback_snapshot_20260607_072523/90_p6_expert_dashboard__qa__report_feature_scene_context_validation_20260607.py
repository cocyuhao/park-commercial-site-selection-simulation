...
def main() -> int:
    ...
    try:
        ...
        if derivative_id:
            ...
            job_response = client.post(
                "/api/simulation/jobs",
                json={"scenario_name": "qa_report_accuracy_context", "seed": 2026060701, "iterations": 25},
            )
            checks.append(check(job_response.status_code == 200, "API-SIM-JOB-CREATE", "报告验证可创建一次结构化仿真检查。", evidence=job_response.status_code))
            job_payload = job_response.json() if job_response.status_code == 200 else {}
            job_id = str((job_payload.get("job") or {}).get("job_id") or "")
            checks.append(check(bool(job_id), "API-SIM-JOB-ID", "结构化仿真检查返回稳定 job 编号。", evidence=job_id))
            ...
            accuracy_context = report_payload.get("simulation_accuracy_context") or {}
            ...
            checks.extend(
                [
                    ...
                    check(int(accuracy_context.get("result_count", 0)) >= 1, "API-REPORT-ACCURACY-CONTEXT", "最近一次仿真准确性上下文进入 dashboard 报告。", evidence={"job_id": accuracy_context.get("job_id"), "result_count": accuracy_context.get("result_count")}),
                    check(any(item.get("name") == "收入与消费能力" for item in accuracy_context.get("constraints", [])), "API-REPORT-ACCURACY-INCOME", "报告准确性约束包含收入与消费能力。", evidence=accuracy_context.get("constraints", [])[:4]),
                    check(any(item.get("income_price") for item in accuracy_context.get("items", [])), "API-REPORT-ACCURACY-EXAMPLES", "报告准确性结果样例包含收入/价格解释。", evidence=accuracy_context.get("items", [])[:2]),
                    ...
                    check(any("结构化仿真检查" in item or "仿真检查" in item for item in report_payload.get("simulation_readiness", {}).get("can_run_now", [])), "API-REPORT-ACCURACY-READINESS", "仿真准确性上下文改变报告的可运行事项说明。", evidence=report_payload.get("simulation_readiness", {}).get("can_run_now", [])[:7]),
                    ...
                    check(any("仿真准确性" in item or "街道级收入" in item for item in report_payload.get("next_actions", [])[:4]), "API-REPORT-ACCURACY-NEXT-ACTION", "仿真准确性约束进入当前推进事项。", evidence=report_payload.get("next_actions", [])[:4]),
                ]
            )
            ...
            report_accuracy_context = report_api_payload.get("report", {}).get("simulation_accuracy_context") or {}
            ...
            checks.append(check(int(report_accuracy_context.get("result_count", 0)) >= 1 and any(item.get("name") == "收入与消费能力" for item in report_accuracy_context.get("constraints", [])), "API-SITE-REPORT-ACCURACY-CONTEXT", "报告生成接口返回仿真准确性上下文与收入约束。", evidence={"job_id": report_accuracy_context.get("job_id"), "constraints": report_accuracy_context.get("constraints", [])[:3]}))
            ...
            if docx_exists and docx_path:
                ...
                checks.extend(
                    [
                        ...
                        check("仿真准确性上下文" in text and "准确性约束" in text and "收入与消费能力" in text, "DOCX-SIM-ACCURACY-SECTION", "DOCX 包含仿真准确性上下文、准确性约束和收入约束。", evidence="仿真准确性上下文/准确性约束/收入与消费能力"),
                    ]
                )
            ...
            checks.extend(
                [
                    ...
                    check("仿真准确性上下文与校准约束" in md_text and "收入与消费能力" in md_text, "MD-SIM-ACCURACY-SECTION", "Markdown 报告包含仿真准确性上下文与收入约束。", evidence=str(md_path)),
                ]
            )
    finally:
    ...
    checks.extend(
        [
            ...
            check("simulation_accuracy_context" in app_js_text and "仿真准确性上下文与校准约束" in app_js_text, "UI-REPORT-SIM-ACCURACY", "前端报告页读取并展示仿真准确性上下文。"),
    ...
    output = {
        ...
        "checks": checks,
        "scope": "验证采用/锁定人物场景、真实校准输入和最近一次仿真准确性上下文是否进入报告、DOCX、Markdown 和 DeepSeek prompt；不证明完整仿真已完成。",
    }
    ...
    lines = [
        ...
        f"- 验证仿真 job：{job_id}",
        "- 边界：本验证只证明用户采用/锁定的人物场景、真实校准输入和仿真准确性上下文进入报告链路，不证明完整人群仿真完成。",
        "",
        "## 检查项",
    ]
    for item in checks:
