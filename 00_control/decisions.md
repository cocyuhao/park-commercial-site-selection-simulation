# DEC-066 P6 空间地图必须使用真实高德 JS 地图

- 日期：2026-06-03
- 决策：P6 空间地图主体验使用高德 JS API 2.0 渲染，不再用静态图或自定义缩放层假装交互地图。
- 原因：用户明确指出地图空白、缩放闪烁、不是高德可拖拽地图、POI 不直观等问题；真实地图体验必须由高德 JS 地图承担。
- 当前实现：前端加载高德 JS API，使用高德地图拖拽、滚轮缩放、按钮缩放、`resize`、`setFitView`、Marker 和 InfoWindow；后端只提供 JS 配置、搜索上下文和周边 POI。
- 边界：地图点位、候选节点和 POI 仍为 `needs_review / not_final`，不得解释为 DWG 坐标、面积、图层或真实动线；不把高德 Key 写进代码、报告或日志。
- 后续校验：本轮 `node --check`、`py_compile`、Selenium + Chrome 10 轮点击测试通过；项目总门禁因本机缺少 `gh` 命令未完成。
...
# 2026-06-03 地图/资料/节点/总览状态决策

- 决策：地图搜索 loading、图层 loading、错误、当前图层、正在搜索关键词和最后一次成功地图上下文必须分开维护。
- 原因：搜索中切换 POI tab 时，如果共用一个 loading 或直接清空列表，会造成结果区空白、错误消失或加载结束后 tab 内容不一致。
- 决策：资料导入按“项目资料链”呈现，采用/放弃/删除是资料本身状态，不再自动弹出抽屉遮挡主流程。
- 决策：节点草案只标记“待复核”，不伪装为确定结论；新增节点与计划生成节点都进入可编辑草案池。
- 边界：高德 JS Key 仍只来自环境变量；报告和代码不得写完整 Key。

# 2026-06-03 GitHub 同步与 LFS 门禁决策

- 决策：项目总门禁的 GitHub 检查改为检查当前仓库 `origin`、`HEAD...origin/main` 同步状态和 Git LFS 跟踪文件，不再依赖本机 `gh` 或匿名扫描外部 fork。
- 原因：本轮普通 Git 已同步，但两个 DWG 仍是 LFS 指针；真正影响当前项目的是 LFS 大文件是否落地，而不是外部归档 fork 是否能被匿名 API 逐项访问。
- 决策：门禁承认 `.env` 中已被 `load_local_env()` 支持的变量别名：`OPENAI_API_KEY` 可映射为 `DEEPSEEK_API_KEY`，`AMAP_API_KEY` 可映射为 `AMAP_WEB_SERVICE_KEY`。
- 边界：DWG 大文件落地只表示源文件同步完成；几何、图层、面积、坐标和动线仍必须等待可信转换产物。

# 2026-06-03 用户流程交互决策

- 决策：地图搜索结果只更新地图数据和状态，不再在异步完成后强制切回地图页。
- 原因：用户可能已经切到资料或 AI 页面，异步搜索结果抢页面会打断当前任务。
- 决策：URL hash 是正式导航状态，必须监听 `hashchange` 并让页面与地址一致。
- 决策：AI 报告生成必须要求当前会话已有消息；空会话只能先对话，不能导出报告。
- 决策：从项目计划生成节点草案必须幂等，同名位置和业态不重复生成。
- 边界：历史测试缓存不在代码里硬删，涉及数据清理时需要用户确认。
