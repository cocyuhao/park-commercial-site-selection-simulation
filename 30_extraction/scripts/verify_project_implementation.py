...
def verify_row_counts() -> None:
    ...
    handoff_encoding_review = check_csv_count("40_quality_evidence/handoff_encoding_health_review.csv", 43, group="files")
    ...
    if p2_source_catalog:
        ...
        add("p2", "error", ok(status_counts.get("pending_conversion", 0) == 2), f"P2 pending_conversion sources={status_counts.get('pending_conversion', 0)}", "40_quality_evidence/p2_real_site_source_catalog.csv")
        ...
        add("p2", "error", ok(all(row.get("p2_use_status") == "pending_conversion_before_geometry_or_layer_use" for row in dwg_rows)), "P2 DWG geometry use remains blocked before conversion", "40_quality_evidence/p2_real_site_source_catalog.csv")
...
def verify_secret_and_encoding() -> None:
    ...
    for path in ROOT.rglob("*"):
        ...
        if "???" in text and path.name != "verify_project_implementation.py":
            if all('rg "???"' not in line and "`???`" not in line for line in text.splitlines() if "???" in line):
